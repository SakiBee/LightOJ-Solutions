...
int main()
{
    ...
    while(tt--)
    {
        ...
        cin >> r >> c;
        CASE(t);
        if(r == 1 || c == 1) coutl(max(r,c));
        else if(r == 2 || c == 2)
        {
            p = max(r,c);
            q = p%4;
            if(q == 2) ans = p+2;
            else if(q == 1 || q == 3) ans = p+1;
            else ans = p;
        ...
        {
            ans = c*r;
            (ans&1) ? coutl((ans/2)+1) : coutl(ans/2);
        }
...
//http://programming-jinnatul.blogspot.com/2018/10/lightoj-1010-knights-in-chessboard.html
