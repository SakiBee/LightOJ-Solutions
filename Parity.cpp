...
ll decToBinary(ll n){int binaryNum[32], i = 0, cnt = 0;while (n > 0) {binaryNum[i] = n % 2;n = n / 2;i++;}
    for (ll j = i - 1; j >= 0; j--)if(binaryNum[j] == 1)cnt++;return cnt;}
...
int main()
{
    ...
    {
        ll n; cin >> n;
        ll one = decToBinary(n);
        CASE(t);
        one & 1 ? coutl("odd") : coutl("even");
