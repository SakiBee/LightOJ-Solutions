...
ll bin_to_dec(string s){ll ans = 0;for(ll i=s.size()-1,j=0; i>=0; j++,i--)ans += (s[i]-'0')* pow(2,j);return ans;}
...
int main()
{
    ...
    while(tt--)
    {
        string s1,s2,as1,bs1,cs1,ds1,as2,bs2,cs2,ds2;
        ll a1,b1,c1,d1,a2,b2,c2,d2,n;
        cin >> s1 >> s2;
        vll v1,v2;
        ll sz1 = s1.size();
        ll sz2 = s2.size();
        for0(i,sz1) if(s1[i] == '.') v1.pb(i);
        for0(i,sz2) if(s2[i] == '.') v2.pb(i);

        as1 = s1.substr(0,v1[0]);                 a1 = stoll(as1);
        bs1 = s1.substr(v1[0]+1,(v1[1]-v1[0])-1); b1 = stoll(bs1);
        cs1 = s1.substr(v1[1]+1,(v1[2]-v1[1])-1); c1 = stoll(cs1);
        ds1 = s1.substr(v1[2]+1,(sz1-v1[2])-1);   d1 = stoll(ds1);

        as2 = s2.substr(0,v2[0]);
        bs2 = s2.substr(v2[0]+1,(v2[1]-v2[0])-1);
        cs2 = s2.substr(v2[1]+1,(v2[2]-v2[1])-1);
        ds2 = s2.substr(v2[2]+1,(sz2-v2[2])-1);
        ...
        d2 = bin_to_dec(ds2);
        CASE(t);
        if(a1 == a2 && b1 == b2 && c1 == c2 && d1 == d2) yes;
        else no;
