https://lightoj.com/problem/k-sat-problem
...
//ll numDiv(ll n){ll cnt = 0; for(ll i=1; i*i<n; i++) if(n%i == 0) (n/i == i) ? cnt++ : cnt+=2; return cnt;}
...
ll numDiv(ll n){ll sqrtn = sqrt(n);ll res = 1;for(ll i=0; i<primes.size() && primes[i] <= sqrtn; i++){if(n%primes[i] == 0){ll p = 0;while(n%primes[i] == 0){n /= primes[i];p++;}sqrtn = sqrt(n);p++;res *= p;}}if(n != 1){res *= 2;}return res;}
#define mod 10000007
...
int main()
{
    ...
    while(tt--)
    {
        ...
        for0(i,n)
        {
            ...
            for0(j,k)
            {
                if(v[i][j] > 0 and check[v[i][j]]){
                    y = 1; break;
                }
                else if(v[i][j] <0 and check[-v[i][j]] == 0){
                    y = 1; break;
                }
            }
            if(y) q++;
